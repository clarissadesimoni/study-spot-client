...
function TaskComponent({ obj, editFunc, closeFunc, deleteFunc }) {
    ...
    return (
        <>
            ...
            <text>{obj.name} - {projects[obj.projectId]} - {obj.due ? obj.due.toISOString() : 'no due date'} - {obj.labels ? obj.labels.map(label => labels[label]).reduce((acc, label) => acc + ', ' + label) : 'no labels'}</text>
